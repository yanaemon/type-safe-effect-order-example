...
class UserService<S extends State = "draft"> {
    ...
    isSaved(): this is UserService<"saved"> {
    ...
    assertValidated(): asserts this is UserService<"validated"> {
        if (!this.isValidated()) {
...
async function processIfValidated(s: UserService<"draft" | "validated">) {
    if (s.isValidated()) {
...
async function processOrThrow(s: UserService<"draft" | "validated">) {
    s.assertValidated();
...
// -----------------------------------------------------------------------------
// ユースケース 3: 「外から来た値」を narrow する典型 (DB / JSON / Queue)
// -----------------------------------------------------------------------------
//
// `loadFromAnywhere` は runtime にしか状態が分からない (= union 型で返す)。
// この場合、03 の `this:` 制約だけでは進めなくて、predicate / assertion が
// 「外の世界と型の世界を繋ぐ」橋渡しになる。

declare function loadFromAnywhere(): UserService<State>;
...
const draft: UserService<"draft" | "validated"> = new UserService({
    name: "test",
    age: 30,
});
...
async function _typeOnlyExamples(s: UserService<"draft" | "validated">) {
