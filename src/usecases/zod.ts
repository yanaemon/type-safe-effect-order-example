...
// `z.infer<>` は phantom output 型を取り出す道具。
// → 02 で手書きした ValidatedUserData と同じ役割の型がここで生える。
...
async function saveUser(user: ValidatedUserData): Promise<void> {
    console.log("[save]   ", user.name);
}

async function notifyUser(user: ValidatedUserData): Promise<void> {
    console.log("[notify] ", user.name);
}

// ----- ✅ 正しい使い方: parse を通すと型が ValidatedUserData に昇格 ----------
{
    const raw: unknown = { name: "test", age: 30 };
...
const BrandedUserSchema = UserSchema.brand<"validated">();
type BrandedUser = z.infer<typeof BrandedUserSchema>;
//   ^? { name: string; age: number } & z.$brand<"validated">

async function saveBranded(user: BrandedUser): Promise<void> {
    console.log("[save*]  ", user.name);
}
...
// -----------------------------------------------------------------------------
// 5) zod 単体では「validate → save → notify」の順序は守れない
// -----------------------------------------------------------------------------
//
// `.brand()` で得られるのは「validate を通った」という 1 つのラベルだけ。
// save の後に notify を呼ぶ、save の前に notify を呼ぶ、を型で区別するには、
//   - 02-phantom-pipeline.ts のようにラベルを段ごとに足す
//   - 03-type-state-pattern.ts のように箱 (class) に状態を持たせる
// のどちらかが要る。zod はその「最初の段」を提供してくれる便利装置と捉えると分かりやすい。

type SavedUser = BrandedUser & { readonly __saved: true };

async function saveAndTag(user: BrandedUser): Promise<SavedUser> {
    console.log("[save*]  ", user.name);
    return user as SavedUser;
}

async function notifySaved(user: SavedUser): Promise<void> {
    console.log("[notify*]", user.name);
}
