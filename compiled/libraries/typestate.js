...
var UserState;
(function (UserState) {
    UserState[UserState["Draft"] = 0] = "Draft";
    UserState[UserState["Validated"] = 1] = "Validated";
    UserState[UserState["Saved"] = 2] = "Saved";
    UserState[UserState["Notified"] = 3] = "Notified";
})(UserState || (UserState = {}));
// -----------------------------------------------------------------------------
// 遷移表を declare する小ヘルパー (毎回同じセットアップを使い回すため)
// -----------------------------------------------------------------------------
function createFsm() {
    const fsm = new TypeState.FiniteStateMachine(UserState.Draft);
