...
const { nextState } = stateMachine()
    ...
    return { stateName: "validated" };
})
    .actionHandler("validated", "SAVE", (_state, _action) => ({ stateName: "saved" }))
    .actionHandler("saved", "NOTIFY", (_state, _action) => ({ stateName: "notified" }))
    .done();
// -----------------------------------------------------------------------------
...
console.log("--- happy path ---");
{
    ...
    let state = nextState({ stateName: "draft" }, {
        actionName: "VALIDATE",
        input,
    });
...
{
    const bad = { name: "", age: 30 };
    const state = nextState({ stateName: "draft" }, {
        actionName: "VALIDATE",
        input: bad,
    });
