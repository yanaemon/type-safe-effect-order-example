...
const save = (input) => ResultAsync.fromPromise((async () => {
    console.log("[save]   ", input.name);
    return input;
})(), (cause) => new SaveError(cause));
const notify = (input) => ResultAsync.fromPromise((async () => {
...
const program = (input) => validate(input) //                                  : Result<UserData, ValidationError>
    .asyncAndThen(save) //                          : ResultAsync<UserData, ValidationError | SaveError>
    .andThen(notify); //                            : ResultAsync<void,     ValidationError | SaveError | NotifyError>
// -----------------------------------------------------------------------------
...
await program({ name: "test", age: 30 }).match(() => console.log("[done]"), (e) => console.log("[unreachable]", e));
console.log("--- validation failure ---");
await program({ name: "", age: 30 }).match(() => console.log("[unreachable]"), (e) => {
    // _tag による narrowing
    switch (e._tag) {
...
const validateBranded = (input) => {
    if (input.name.length === 0 || input.age < 0) {
        return err(new ValidationError("invalid input"));
    }
    return ok(input);
};
const saveBranded = (input) => ResultAsync.fromPromise((async () => input)(), (cause) => new SaveError(cause));
