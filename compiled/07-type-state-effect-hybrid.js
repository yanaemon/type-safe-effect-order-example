...
const program = pipe(userService.validate(), Effect.flatMap((s) => s.save()), Effect.flatMap((s) => s.notify()));
...
console.log("--- failure ---");
const failedProgram = pipe(new UserService({ name: "", age: 30 }).validate(), Effect.flatMap((s) => s.save()), Effect.flatMap((s) => s.notify()));
